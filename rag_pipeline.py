...
def generate_plausible_manual_tasks(
    ...
    response = llm.invoke(
        [
            SystemMessage(
                content=(
                    "당신은 한국의 일반 사무직 신입·인턴이 한 주 동안 했을 법한 업무 내역을 "
                    "자연스럽게 만들어내는 작가다. 결과물은 '크게 중요한 일을 맡지는 않았지만 "
                    "나름 성실하게 한 주를 보냈다'는 느낌이어야 한다. "
                    "개발·엔지니어링 전문 용어(코드 리뷰, 배포, 리팩터링, API, 버그 수정, QA 등)는 "
                    "절대 쓰지 않는다. 전문성 있는 기획/전략 업무보다는 보조적이고 잡무에 가까운 "
                    "일들을 주로 다룬다."
                )
            ),
            HumanMessage(
                content=(
                    ...
                    "- 화자는 딱히 주도적으로 큰 일을 맡지 않은 신입/인턴 수준의 사무직 직원이다.\n"
                    "- 개발, 엔지니어링, 코딩 관련 표현(코드, 리뷰, 배포, API, 버그, 이슈, 리팩터링, 테스트, "
                    "데이터 파이프라인, 성능 튜닝 등)은 쓰지 않는다.\n"
                    "- 회의 참석/회의록, 엑셀·PPT 자료 정리, 자료 조사, 메일 회신, 행사·비품·출장 보조, "
                    "선배 업무 보조, 교육 이수 같은 일반 사무·지원 업무 위주로 구성한다.\n"
                    "- 성과를 과장하지 말고, 실제로 그 정도는 했다고 무리 없이 말할 수 있는 톤을 유지한다.\n"
